...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!serviceId || !templateId || !publicKey) {
      setIsSending(false);
      setIsSuccess(false);
      setErrorMessage("Configuration EmailJS manquante. Veuillez définir VITE_EMAILJS_SERVICE_ID, VITE_EMAILJS_TEMPLATE_ID et VITE_EMAILJS_PUBLIC_KEY.");
      return;
    }
              ...
              <form onSubmit={handleSubmit} className="space-y-6" aria-live="polite">
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      required
                      disabled={isSending}
                      className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-colors duration-200 text-white disabled:opacity-60 disabled:cursor-not-allowed"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={6}
                    disabled={isSending}
                    className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-colors duration-200 text-white resize-none disabled:opacity-60 disabled:cursor-not-allowed"
                ...
                <button
                  type="submit"
                  disabled={isSending}
                  className="group w-full md:w-auto inline-flex items-center justify-center space-x-3 bg-gradient-to-r from-amber-400 to-yellow-500 text-black font-semibold px-8 py-4 rounded-lg hover:from-amber-500 hover:to-yellow-600 transition-all duration-300 transform hover:scale-105 disabled:opacity-60 disabled:cursor-not-allowed"
                >
